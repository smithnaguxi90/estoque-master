...
const INITIAL_MATERIALS = [
  ...
  },
  {
    ...
    contractCode: "B.04.07.003",
    sku: "39999",
    name: 'PERFIL EM ALUMÍNIO TIPO "U" 5/8 X 1"- CÓDIGO OSO: 24527',
    category: "DEMAP - 01 ao 11",
    quantity: 0,
    minQuantity: 10,
    alertPercentage: 40,
...
// ... (Resto dos Helpers: getResupplyAlertLevel, getMaterialStatus, renderTable) ...
    ...
    if (allowSort && col.sortKey) {
      ...
      th.onclick = () => {
        ...
        if (current.key === col.sortKey) {
          state.sort[paginationKey].asc = !current.asc;
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  el.skuInputAdd.addEventListener("input", () => {
    ...
    if (isDuplicate) {
      ...
      el.skuFeedback.textContent = "Código já existe! Dados carregados.";
    } else {
  ...
  el.materialForm.addEventListener("submit", (e) => {
    ...
    if (id) {
      ...
      if (existing) {
        ...
        mat.contractCode = existing.contractCode; // Preserve existing contract code
      }
    }
  ...
  el.logoutBtn.addEventListener("click", async (e) => {
    // Logout (recarregar a página limpa o estado em aplicações sem backend real)
    location.reload();
  });
